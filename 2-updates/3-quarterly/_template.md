# Q# YYYY (e.g. Q3 2026)

> Non-technical quarterly CTO review — rolls up the quarter's monthly updates + roadmap progress.
> Board-level view for the executives. Drafted by `/quarterly` — **confirm every [bracketed] number
> before sending.**

## TL;DR
_The quarter in a few sentences._

## Roadmap progress
_Planned vs delivered this quarter (derive from [[next-actions]], [[backlog]] and [[ideas]])._
- **Planned this quarter:**
- **Delivered:**
- **Slipped / carried over:**

## Metrics (this quarter + cumulative)
- **Discovery:** systems documented [x] of ~[N]
- **Delivery:** shipped [ ] · issues resolved [ ]
- **Risk reduction:** key-person risk [status] · backups/DR [status]
- **Time & money:** hours saved [ ] · cost saved/avoided [ ]  (cumulative to date: [ ])

## Wins delivered (savings: hours & £)
-

## Risk register status
_Top risks and where they stand._
-

## Opportunities & what's next
_From [[ideas]]._
-

## Team & resourcing needs
_What the function needs to keep delivering (people, tools, budget)._
-

## Decisions
- **Made:**
- **Need from you:**

## Next quarter — focus
-

---

## 📧 For Jack Bull (Director — who I report to)
_Strategic: progress vs the vision/roadmap, business impact, the resourcing ask (if any). **Name the tools.**_

> Hi Jack,
>
> [Quarter's story in 4–6 sentences: outcomes, momentum, where we are on the roadmap.]
>
> [Team/resourcing ask, or "nothing needed".]
>
> — Jonathan

## 📧 For Dee (CFO)
_Numbers-first: cumulative savings & risk reduction this quarter, spend, ROI narrative — **name tools/spend**._

> Hi Dee,
>
> [Cumulative hours/£ saved and risk reduced this quarter; ROI vs any spend.]
>
> [Any budget ask, or "nothing needed".]
>
> — Jonathan
