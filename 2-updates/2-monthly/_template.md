# Month YYYY-MM (Month Name)

> Non-technical monthly CTO update — rolls up the month's weekly updates for the executives.
> The two messages at the bottom are ready to send. Drafted by `/monthly` — **confirm every
> [bracketed] number before sending.** Answers the Monthly Review questions in
> [[technology-strategy-2026-2028]].

## TL;DR
_The month in 2–3 sentences._

## Highlights — what changed for the business
_Plain-English outcomes, not how they were built._
-

## Metrics (this month, trend vs last month ▲/▼/→)
- **Discovery:** systems documented [x] of ~[N]  ([trend])
- **Delivery:** shipped [ ] · issues resolved [ ]
- **Risk reduction:** key-person knowledge captured [ ] · backups/DR [status]
- **Time & money:** hours saved [ ] · cost saved/avoided [ ]

## Wins (with hours/£ where known)
-

## What I learned
-

## Biggest risks & mitigation
-

## Biggest opportunities
_From [[ideas]] — what's worth pursuing._
-

## Decisions
- **Made:**
- **Need from you:**

## Next month — priorities
-

---

## 📧 For the CEO
_Strategic & warm: the month's headline outcomes, momentum toward the vision, one risk handled, any ask._

> Hi [CEO],
>
> [3–5 sentences: what moved the business this month and where we're heading.]
>
> [One risk being managed.]  [Any ask, or "nothing needed this month".]
>
> — Jonathan

## 📧 For the CFO
_Numbers-first: efficiency, cost, continuity. Savings to date, any spend, any ask._

> Hi [CFO],
>
> [Lead with the numbers: hours/£ saved this month, continuity-risk status.]
>
> [Any spend or upcoming cost.]  [Any ask, or "nothing needed this month".]
>
> — Jonathan
